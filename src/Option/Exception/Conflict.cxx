#line 2 "libsxc:Option/Exception/Conflict.cxx"
// LICENSE/*{{{*/
/*
  libsxc
...
namespace libsxc
{
  namespace Option
  {
    namespace Exception
    {
      Conflict::Conflict(const char* name) throw()/*{{{*/
      : libsxc::Exception::Exception()
      {
        _createMessage(name);
        setType(libsxc::Exception::Conflict);
        createBacktrace();
      }
...
/*}}}*/
      void Conflict::_createMessage(const char* name) throw()/*{{{*/
      {
        char messageBuffer[libsxc::Exception::Exception::MESSAGE_BUFFER_SIZE];
        const char* format = "Conflicting option(s): '%s'";
