#line 2 "libsxc:Option/Exception/UnknownOption.cxx"
// LICENSE/*{{{*/
/*
  libsxc
...
namespace libsxc
{
  namespace Option
  {
    namespace Exception
    {
      UnknownOption::UnknownOption(const char* name) throw()/*{{{*/
...
/*}}}*/
      void UnknownOption::_createMessage(const char* name) throw()/*{{{*/
      {
        char messageBuffer[libsxc::Exception::Exception::MESSAGE_BUFFER_SIZE];
        const char* format = "Unknown option or value: '%s'";
