#line 2 "libsxc:Option/Exception/MissingValue.cxx"
// LICENSE/*{{{*/
/*
  libsxc
...
namespace libsxc
{
  namespace Option
  {
    namespace Exception
    {
      MissingValue::MissingValue(const char* name) throw()/*{{{*/
      : libsxc::Exception::Exception()
      {
        _createMessage(name);
        setType(libsxc::Exception::MissingValue);
        createBacktrace();
      }
...
/*}}}*/
      void MissingValue::_createMessage(const char* name) throw()/*{{{*/
      {
        char messageBuffer[libsxc::Exception::Exception::MESSAGE_BUFFER_SIZE];
        const char* format = "This option needs a value: '%s'";
